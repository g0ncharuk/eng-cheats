...
function getMessage(percent: number) {
  if (percent >= 90) return 'Чудово! Phrasal verbs для тебе — як рідні.'
  if (percent >= 70) return 'Добре! Ще трохи практики і буде ідеально.'
  if (percent >= 50) return 'Непогано, але деякі phrasal verbs варто повторити.'
  return 'Не здавайся! Переглянь довідник і спробуй знову.'
}

export function PhrasalResult({ correct, total, onRetry }: {
  correct: number
  total: number
  onRetry: () => void
}) {
