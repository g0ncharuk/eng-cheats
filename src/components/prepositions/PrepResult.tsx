...
function getMessage(percent: number) {
  if (percent >= 90) return 'Чудово! In, on, at — для тебе як рідні.'
  if (percent >= 70) return 'Добре! Ще трохи і помилок не буде.'
  if (percent >= 50) return 'Непогано, але деякі правила варто повторити.'
  return 'Не здавайся! Переглянь шпаргалку зверху і спробуй знову.'
}

export function PrepResult({ correct, total, onRetry }: {
  correct: number
  total: number
  onRetry: () => void
}) {
