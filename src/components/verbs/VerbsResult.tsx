...
function getMessage(percent: number) {
  if (percent >= 90) return 'Чудово! Ти знаєш ці дієслова на відмінно.'
  if (percent >= 70) return 'Добре! Ще трохи практики і буде ідеально.'
  if (percent >= 50) return 'Непогано, але варто повторити деякі дієслова.'
  return 'Не здавайся! Спробуй режим "Вивчення" для повторення.'
}

export function VerbsResult({ correct, total, bestStreak, onRetry }: {
  correct: number
  total: number
  bestStreak: number
  onRetry: () => void
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-8">
        ...
        {bestStreak > 1 && (
          <p className="text-amber-400 text-sm mb-2">
            {'\u{1F525}'} Найдовша серія: {bestStreak} поспіль
          </p>
        )}
